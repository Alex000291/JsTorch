...
const sources = {
    ...
    cuda: ['native/ops/unary.cu', 'native/ops/binary.cu', 'native/ops/reduce.cu', 'native/audio/stft.cu']
};
...
for (const src of sources.cuda) {
    ...
    console.log(`  ${src}`);
    try {
        execSync(`nvcc -std=c++17 -O3 --use_fast_math -Xcompiler /MD -Xcompiler /EHsc -DNAPI_VERSION=8 --generate-code=arch=compute_89,code=sm_89 --generate-code=arch=compute_90,code=sm_90 --generate-code=arch=compute_90,code=compute_90 ${includes} -c ${src} -o ${obj}`, { stdio: 'inherit' });
    } catch(e) {
...
for (const src of sources.cpp) {
    ...
    console.log(`  ${src}`);
    try {
        const output = execSync(`cl /std:c++17 /O2 /MD /EHsc /utf-8 /DNAPI_VERSION=8 ${includes} /c ${src} /Fo${obj}`, { encoding: 'utf8', stdio: 'pipe' });
...
execSync(`link /DLL /OUT:build/jstorch.node ${objs} /LIBPATH:"build\\win\\libs\\node" /LIBPATH:"${CUDA_PATH}\\lib\\x64" node.lib cudart.lib cublas.lib cufft.lib`, { stdio: 'inherit' });
